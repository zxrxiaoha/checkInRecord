...
class App {
  ...
  initEventListeners() {
    ...
    document.getElementById("searchRecords").addEventListener("click", () => {
      ...
      const filteredRecords = recordManager.searchRecords(
        checkIn.getAllRecords(),
        {
          startDate,
          endDate,
          keyword,
    ...
    document.getElementById("clearRecords").addEventListener("click", () => {
      modal.confirm("确定要清空所有打卡记录吗？此操作不可恢复！", () => {
    ...
    // 分享成就按钮事件
    document
      .getElementById("shareAchievement")
      .addEventListener("click", () => {
        share.previewAchievement();
      });
    // 生成测试数据
    document.getElementById("createRecords").addEventListener("click", () => {
  ...
  updateCalendar() {
    ...
    calendarContainer.innerHTML = calendar.generateCalendar(
      checkIn.getAllRecords()
    );
  }
