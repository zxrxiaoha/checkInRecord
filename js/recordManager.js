...
class RecordManager {
    ...
    generateTestData(count = 1000) {
        const records = [];
        const contents = ['开发任务', '需求评审', '代码审查', '项目会议', '技术分享', '日常工作'];
        const now = new Date();
        let lastEndTime = now;
        
        for (let i = 0; i < count; i++) {
            // 确保每条记录的开始时间晚于上一条记录的结束时间
            const date = new Date(lastEndTime.getTime() - Math.random() * 24 * 60 * 60 * 1000);
            const hours = Math.floor(Math.random() * 8) + 1;
            const minutes = Math.floor(Math.random() * 60);
            const seconds = Math.floor(Math.random() * 60);
            const duration = `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
            const endTime = new Date(date.getTime() + (hours * 3600000 + minutes * 60000 + seconds * 1000));
